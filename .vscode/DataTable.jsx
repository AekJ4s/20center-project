...
export default function DataTable() {
  ...
  const filterItems = (item) => {
    ...
    const searchText = textSeacrh.toLowerCase()
    const categoryMatch =
      Category === '' || productType === Category.toLowerCase()
    return productName.includes(searchText) && categoryMatch
  }
  const filteredItems = currentItems.filter(filterItems)
  console.log("ฟิลเตอร์"+filteredItems)
  
  const handleChange = (event) => {
    setCategory(event.target.value)
    settextSeacrh(event.target.value)
  }
